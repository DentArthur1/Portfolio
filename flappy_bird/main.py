...
class Bird():
    ...
    def __init__(self):
        self.bird_img = pygame.image.load("imgs/flappy_bird.png").convert_alpha()
...
class Game():
    ...
    def __init__(self):
        ...
        self.restart_ = self.font.render("Restart", True, (255,255,255))
        self.restart_rect = self.restart_.get_rect()
        self.FPS = 75
        self.times = 0
        self.gravity_coeff = 0.21
        self.running = True
        self.bird = Bird()
    ...
    def update_screen(self):
            ...
            self.bird.bird_rect, self.bird.bird_y,self.scenary.background_speed, self.scenary.floor_speed,self.bird.count = self.bird.check_collisions(self.scenary.obstacles, self.bird.bird_rect, self.bird.bird_y, self.scenary.background_speed, self.scenary.floor_speed,self.bird.count)
            self.bird.blit_bird(self.bird.inclination,  self.bird.bird_img, self.bird.bird_x, self.bird.bird_y,self.screen)
    ...
    def restart(self):
        ...
        start = 0
        count = 0
        bg_speed, floor_speed = 0.5,2
        dead = False
        x, y = 200, 200
        rot_coeff, max_vel, incl = 2, 4, 20
        return obstacles, start, count, bg_speed, floor_speed, dead, x ,y, rot_coeff, max_vel, incl
    ...
    def main_loop(self):
        ...
        while self.running:
                ...
                if event.type == pygame.MOUSEBUTTONUP:  #checks for mouse input 
                        if self.bird.dead:
                            if self.restart_rect.collidepoint(pygame.mouse.get_pos()):
                                self.scenary.obstacles, self.times, self.bird.count, self.scenary.background_speed, self.scenary.floor_speed,self.bird.dead,self.bird.bird_x, self.bird.bird_y,self.bird.rot_angle_coeff, self.bird.max_vel, self.bird.inclination = self.restart()
                        else:
            ...
            if self.bird.dead:
                self.screen.blit(self.restart_, (130,200))
